                ...
                <section class="admin-single" id="media" data-media-root>
                    <div class="admin-card media-section-switcher" data-media-section-switcher data-initial-section="<?= escape($activeMediaSection) ?>">
                    ...
                    <div class="media-section-panel" data-media-section-panel="profile"<?= $activeMediaSection === 'profile' ? '' : ' hidden' ?>>
                        <div class="admin-layout media-admin-layout">
                            <div class="admin-card">
                                ...
                                <p class="form-hint">Aktuální serverový limit pro jeden nahrávaný soubor je <strong><?= escape($uploadMaxFilesize) ?></strong>.</p>
                            </div>
                                        ...
                                        <?php foreach ($profileMedia as $item): ?>
                                            <article class="media-admin-card media-admin-profile-card">
                                                <div class="media-admin-profile-visual">
                                                    <img src="<?= escape((string) $item['image_path']) ?>" alt="Profilová fotka" loading="lazy" decoding="async">
                                                </div>
                                                ...
                                                <form method="post" class="media-admin-card-actions">
                                                    <?= csrfInputField() ?>
                                                    <input type="hidden" name="media_id" value="<?= escape((string) $item['id']) ?>">
                                                    <input type="hidden" name="media_section" value="profile">
                                        ...
                                        <?php foreach ($galleryMedia as $item): ?>
                                            <article class="media-admin-card">
                                                ...
                                                <div class="media-admin-card-body">
                                                    <strong><?= escape((string) (($item['title'] ?? '') !== '' ? $item['title'] : 'Fotografie galerie')) ?></strong>
                                                    <?php if (($item['subtitle'] ?? '') !== ''): ?>
                                                        <p class="media-admin-card-meta"><?= escape((string) $item['subtitle']) ?></p>
                                                    <?php endif; ?>
                                                    <div class="media-admin-card-badges">
                                                        <span class="media-admin-badge">Pořadí: <?= escape((string) ((int) ($item['sort_order'] ?? 0))) ?></span>
                    ...
                    <div class="media-section-panel" data-media-section-panel="certificates"<?= $activeMediaSection === 'certificates' ? '' : ' hidden' ?>>
                                        ...
                                        <?php foreach ($certificateFiles as $item): ?>
                                            <article class="media-admin-card media-admin-card-certificate">
                                                ...
                                                <div class="media-admin-card-body">
                                                    ...
                                                    <p class="media-admin-card-meta"><?= escape((string) ($item['name'] ?? '')) ?></p>
                                                </div>
                        ...
                        <p class="form-hint">Aktuální serverový limit pro jeden nahrávaný soubor je <strong><?= escape($uploadMaxFilesize) ?></strong>. Pokud se po odeslání nic nestane, bývá soubor větší než povolený limit.</p>
                    </div>
                </section>
