...
function prepareFixture(mysqli $connection, string $token): array
{
    ...
    $date = findFixtureDateWithoutReservations($connection);
    $availabilityIds = [];
    $windows = [
...
function findFixtureDateWithoutReservations(mysqli $connection): string
{
...
function findReservationRow(mysqli $connection, int $reservationId): array
{
    $statement = $connection->prepare(
...
function buildReservationService(mysqli $connection): ReservationService
{
...
$storageDir = ppstudioCliTestTempSecurityStorageDir(SCRIPT_PREFIX, 'ppstudio-admin-reservation-usecase-');
...
try {
    ...
    $reservationRepository = new ReservationRepository($connection);
    $reservationService = buildReservationService($connection);
    ...
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, (bool) ($confirmResult['success'] ?? false), 'Potvrzeni rezervace ma probehnout uspesne.');
    $confirmRow = findReservationRow($connection, $confirmId);
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'potvrzena', (string) ($confirmRow['stav'] ?? ''), 'Rezervace ma byt po potvrzeni ve stavu potvrzena.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'Potvrzeno administrací', (string) ($confirmRow['poznamka_admina'] ?? ''), 'Admin poznamka ma byt ulozena.');

    $cancelId = insertReservation(
        $connection,
        'aut:' . $token . ':c2',
        $fixture['service_id'],
    ...
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, (bool) ($cancelResult['success'] ?? false), 'Zruseni rezervace ma probehnout uspesne.');
    $cancelRow = findReservationRow($connection, $cancelId);
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'zrusena', (string) ($cancelRow['stav'] ?? ''), 'Rezervace ma byt po zruseni ve stavu zrusena.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'Nemůže dorazit', (string) ($cancelRow['duvod_zruseni'] ?? ''), 'Duvod zruseni ma byt ulozen.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'admin_lite', (string) ($cancelRow['zruseno_kym'] ?? ''), 'Lite admin ma byt zapsan do cancel meta.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'lite-review', (string) ($cancelRow['zruseno_uzivatel'] ?? ''), 'Lite admin username ma byt zapsan do cancel meta.');
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, trim((string) ($cancelRow['zruseno_at'] ?? '')) !== '', 'Cas zruseni ma byt vyplnen.');

    $rescheduleId = insertReservation(
        $connection,
        'aut:' . $token . ':c3',
        $fixture['service_id'],
    ...
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, (bool) ($rescheduleResult['success'] ?? false), 'Preplanovani rezervace ma probehnout uspesne.');
    $rescheduleRow = findReservationRow($connection, $rescheduleId);
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, $fixture['date'] . ' 15:00:00', (string) ($rescheduleRow['datum_cas'] ?? ''), 'Rezervace ma byt presunuta na novy termin.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, null, $rescheduleRow['reminder_sent_at'] ?? null, 'Preplanovani ma vynulovat reminder_sent_at.');

    $cancelledMoveId = insertReservation(
        $connection,
        'aut:' . $token . ':c4',
        $fixture['service_id'],
    ...
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, ! (bool) ($cancelledMoveResult['success'] ?? true), 'Presun zrusene rezervace ma byt odmitnut.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 422, (int) ($cancelledMoveResult['http_code'] ?? 0), 'Presun zrusene rezervace ma vratit 422.');

    $completedMoveId = insertReservation(
        $connection,
        'aut:' . $token . ':c5',
        $fixture['service_id'],
    ...
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, ! (bool) ($completedMoveResult['success'] ?? true), 'Presun dokoncene rezervace ma byt odmitnut.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 422, (int) ($completedMoveResult['http_code'] ?? 0), 'Presun dokoncene rezervace ma vratit 422.');
    ...
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, (bool) ($manualSuccessResult['success'] ?? false), 'Rucni rezervace ma jit vytvorit pro volny termin.');
    $manualReservationId = (int) (($manualSuccessResult['data']['reservation_id'] ?? 0));
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, $manualReservationId > 0, 'Rucni rezervace ma vratit reservation_id.');
    $manualRow = findReservationRow($connection, $manualReservationId);
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'potvrzena', (string) ($manualRow['stav'] ?? ''), 'Rucni rezervace se ma ulozit jako potvrzena.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 'manual-success-' . $token . '@example.test', (string) ($manualRow['email'] ?? ''), 'Rucni rezervace ma ulozit e-mail.');
    ...
    ppstudioCliTestAssertTrue(SCRIPT_PREFIX, ! (bool) ($manualCollisionResult['success'] ?? true), 'Kolizni rucni rezervace ma byt odmitnuta.');
    ppstudioCliTestAssertSame(SCRIPT_PREFIX, 422, (int) ($manualCollisionResult['http_code'] ?? 0), 'Kolizni rucni rezervace ma vratit 422.');

    echo SCRIPT_PREFIX . ' [OK] Admin reservation use-case scenarios passed.' . PHP_EOL;
    exit(0);
